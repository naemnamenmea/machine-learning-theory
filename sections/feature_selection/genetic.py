...
class GeneticAlgSelect():
    ...
    def __gen_pool(self):
        ...
        for i in range(self.pool_size):
            gene = np.random.randint(low=0, high=1, size=self.gene_len)
...
class GeneticAlgLocalModel():
    ...
    def _train_mdl(self, data_in, data_out):
        if self.changed:
            ...
            i = 0
            for train_idx,test_idx in kf.split(data_in.shape[0]):
                if (i<1):
...
if __name__ == "__main__":
    ...
    gen_alg = GeneticAlgSelect(X,y,LogisticRegression,params)
    gen_alg._perform_iter()